...
public class SalesOrderService {
  ...
  public SalesOrderDto updateSoStatus(Long soId, String status) {
    ...
    if (salesOrder.getStatus().equals("Đã hoàn thành")) {
      throw new CustomException("Không thể cập nhật đơn bán hàng đã hoàn thành!", HttpStatus.BAD_REQUEST);
    }
    if (salesOrder.getStatus().equals("Đã hủy")) {
      throw new CustomException("Không thể cập nhật đơn bán hàng đã bị hủy!", HttpStatus.BAD_REQUEST);
    }
